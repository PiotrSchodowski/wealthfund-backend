...
CREATE TABLE IF NOT EXISTS wallets_positions (
...
CREATE TABLE IF NOT EXISTS wallet_value_history (
  ...
  date datetime(6) DEFAULT NULL,
  PRIMARY KEY (id),
  KEY key_wallet_value_history_wallet_id (wallet_id),
  CONSTRAINT FK_wallet_value_history_wallet_id FOREIGN KEY (wallet_id) REFERENCES wallets (id)
);
